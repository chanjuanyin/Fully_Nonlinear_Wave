...
if __name__ == "__main__":
    ...
    psi = lambda x1, x2, x3: (8*omega/3)*(torch.exp((4*omega/3) * (1j*x1 + 1j*x2 + 1j*x3)) / (1 + torch.exp((8*omega/3) * (1j*x1 + 1j*x2 + 1j*x3)))) # example initial condition psi(x) = (8*omega/3)*(exp((4*omega/3) * x) / (1 + exp((8*omega/3) * x)))
    ...
    output_file = "complex_d3_results/monte_carlo.csv"
