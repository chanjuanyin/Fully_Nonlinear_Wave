...
def tilde_phi(code, phi, psi, f, z1, a):
    form = code[0]
    alpha_1 = code[1]
    j = code[2]
    if form == 0:
        ...
        return 1. / (torch_factorial_int(alpha_1)) * \
            (
                a**2 * mixed_partial_orders(g = phi, inputs=(z1,), orders=[(0, alpha_1 + 2)]) + \
...
def tilde_psi(code, phi, psi, f, z1, a):
    ...
    if form == 0:
        ...
        return 1. / (torch_factorial_int(alpha_1)) * \
            (
                a**2 * mixed_partial_orders(g = psi, inputs=(z1,), orders=[(0, alpha_1 + 2)]) + \
...
def branching1D(code, phi, psi, f, z, t, a, lambda_):
    ...
    if tau >= t:
        i_1 = (1./2.)*tilde_phi(code, phi, psi, f, z+a*t, a) # compute \frac{1}{2}\tilde{\phi}(code, phi, psi, f, z+a*t)
        i_2 = (1./2.)*tilde_phi(code, phi, psi, f, z-a*t, a) # compute \frac{1}{2}\tilde{\phi}(code, phi, psi, f, z-a*t)
        ...
        if code[0]==1: # if current code is of form \partial^{\alpha}
            ...
            H *= 3 * (1 + alpha)
            if i==1: # if the coordinate in concern is z_1
                H *= ( torch.abs(a)**2 * (2 + alpha) * (3 + alpha) ) / (3 * torch.abs(gamma_1))
...
if __name__ == "__main__":
    ...
    phi = lambda x: 4 * torch.arctan(torch.exp((4*omega/3) * 1j*x)) # example initial condition phi(x) = 4 * arctan(exp(4*omega/3 * ix))
    psi = lambda x: (8*omega/3)*(torch.exp((4*omega/3) * 1j*x) / (1 + torch.exp((8*omega/3) * 1j*x))) # example initial condition psi(x) = (8*omega/3)*(exp(4*omega/3 * ix) / (1 + exp(8*omega/3 * ix)))
    ...
    a = torch.tensor(0.0 + 1.0j, dtype=torch.complex64, device=device) # complex number representing the spatial scaling factor
    ...
    output_file = "complex_d1_results/monte_carlo.csv"
    ...
    for idx, t in enumerate(t_values):
        ...
        result, se_real, se_imag = monte_carlo_simulation(phi, psi, f, z, t.item(), a, lambda_, num_samples)
