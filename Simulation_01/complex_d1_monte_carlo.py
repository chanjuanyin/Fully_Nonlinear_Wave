...
if __name__ == "__main__":
    ...
    phi = lambda x: 4 * torch.arctan(torch.exp((4*w/3) * 1j*x)) # example initial condition phi(x) = 4 * arctan(exp(4*w/3 * ix))
    psi = lambda x: (8*w/3)*(torch.exp((4*w/3) * 1j*x) / (1 + torch.exp((8*w/3) * 1j*x))) # example initial condition psi(x) = (8*w/3)*(exp(4*w/3 * ix) / (1 + exp(8*w/3 * ix)))
    f = lambda u: -(4*(w**2)/3)*torch.sin(u) # example nonlinearity f(u) = -(4*(w**2)/3)*sin(u)
    z = torch.tensor(0.0 + 1.0j, requires_grad=True, device=device) # complex number with requires_grad=True to enable differentiation, representing the initial spatial point z
