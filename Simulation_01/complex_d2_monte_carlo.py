...
if __name__ == "__main__":
    ...
    phi = lambda x1, x2: 4 * torch.arctan(torch.exp((4*omega/3)*(1j*x1 + 1j*x2))) # example initial condition phi(x) = 4 * arctan(exp(2x/3))
    psi = lambda x1, x2: (8*omega/3)*(torch.exp((4*omega/3)*(1j*x1 + 1j*x2)) / (1 + torch.exp((8*omega/3)*(1j*x1 + 1j*x2)))) # example initial condition psi(x) = (4/3)*(exp(2x/3) / (1 + exp(4x/3)))
    ...
    a = torch.tensor(0.0 + (1.0/math.sqrt(2))*1j, dtype=torch.complex64, device=device) # complex number representing the spatial scaling factor
    ...
    output_file = "complex_d2_results/monte_carlo.csv"
