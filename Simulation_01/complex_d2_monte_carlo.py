...
if __name__ == "__main__":
    ...
    phi = lambda x1, x2: 4 * torch.arctan(torch.exp((4*w/3)*(1j*x1 + 1j*x2))) # example initial condition phi(x) = 4 * arctan(exp(2x/3))
    psi = lambda x1, x2: (8*w/3)*(torch.exp((4*w/3)*(1j*x1 + 1j*x2)) / (1 + torch.exp((8*w/3)*(1j*x1 + 1j*x2)))) # example initial condition psi(x) = (4/3)*(exp(2x/3) / (1 + exp(4x/3)))
