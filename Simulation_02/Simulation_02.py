...
def tilde_phi(code, phi, psi, f, z1, z2):
    ...
    j = code[3]
    if form == 1:
        return 1. / (torch.factorial(torch.tensor(alpha_1)) * torch.factorial(torch.tensor(alpha_2))) * \
...
def gradient_tilde_phi(coordinate, code, phi, psi, f, z1, z2):
    if coordinate=='z1':
        ...
        return tilde_phi(new_code, phi, psi, f, z1, z2)
    elif coordinate=='z2':
        new_code = copy.deepcopy(code)
...
def tilde_psi(code, phi, psi, f, z1, z2, a):
    ...
    if form == 1:
        ...
        return 2. / (torch.factorial(torch.tensor(alpha_1)) * torch.factorial(torch.tensor(alpha_2))) * \
...
def branching2D(code, phi, psi, f, z1, z2, t, a, lambda_):
    ...
    if tau >= t:
        r = t * torch.sqrt(1-(1-uniform)**2) # radius of the disk from which we sample the spatial point
        y_1 = a * r * torch.cos(theta) # spatial point z_1 = a * r * cos(theta)
        y_2 = a * r * torch.sin(theta) # spatial point z_2 = a * r * sin(theta)
        i_1 = tilde_phi(code, phi, psi, f, z1+y_1, z2+y_2) # compute \tilde{\phi}(code, phi, psi, f, z1+y_1, z2+y_2)
        i_2 = y_1 * gradient_tilde_phi('z1', code, phi, psi, f, z1+y_1, z2+y_2) + y_2 * gradient_tilde_phi('z2', code, phi, psi, f, z1+y_1, z2+y_2) # compute y_1 * \nabla_{z_1} \tilde{\phi}(code, phi, psi, f, z1+y_1, z2+y_2) + y_2 * \nabla_{z_2} \tilde{\phi}(code, phi, psi, f, z1+y_1, z2+y_2)
        ...
        r = tau * torch.sqrt(1-(1-uniform)**2) # radius of the disk from which we sample the spatial point
        y_1 = a * r * torch.cos(theta) # spatial point z_1 = a * r * cos(theta)
        y_2 = a * r * torch.sin(theta) # spatial point z_2 = a * r * sin(theta)
...
if __name__ == "__main__":
    ...
    phi = lambda x, y: torch.sin(x) * torch.cos(y)
    psi = lambda x, y: torch.cos(x) * torch.sin(y)
    f = lambda u: u**3
